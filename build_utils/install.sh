...
then
	...
	query -a 'BEOS:APP_SIG == "application/x-vnd.Be-POST" || BEOS:APP_SIG == "application/x-vnd.Be-mprf" || 'BEOS:APP_SIG == "application/x-vnd.Be-MAIL"' \
	| grep -v "`/bin/pwd`" \
	| xargs rm
elif [[ $RETURN = "Backup" ]]
then 
	query -a 'BEOS:APP_SIG == "application/x-vnd.Be-POST" || BEOS:APP_SIG == "application/x-vnd.Be-mprf" || 'BEOS:APP_SIG == "application/x-vnd.Be-MAIL" || name == libmail.so' \
	| grep -v "`/bin/pwd`" \
...
rm -f /boot/beos/system/servers/mail_daemon /boot/beos/preferences/E-mail /boot/beos/apps/BeMail
